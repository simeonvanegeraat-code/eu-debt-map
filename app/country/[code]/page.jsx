...
export default function CountryPage({ params: { code } }) {
  ...
  // --- Superstabiele ticker (requestAnimationFrame) ---
  const [nowMs, setNowMs] = useState(Date.now());
  const rafRef = useRef(0);
  useEffect(() => {
    const tick = () => {
      // Gebruik absolute tijd; geen drift als tab wisselt
      setNowMs(Date.now());
  ...
  // Kleine sparkline (alleen voor sfeer)
  const spark = useMemo(() => {
    const pts = 30;
  ...
  return (
    <main className="container grid" style={{ alignItems: "start" }}>
      <section className="card" style={{ gridColumn: "1 / -1" }}>
        ...
        {/* Alleen het totaalbedrag (géén datumbalk) */}
      ...
      <section className="card" style={{ gridColumn: "1 / -1", height: 220 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={spark}>
            ...
            <Tooltip
              formatter={(v) => new Intl.NumberFormat("en-GB").format(Math.round(v))}
              labelFormatter={() => ""}
            />
