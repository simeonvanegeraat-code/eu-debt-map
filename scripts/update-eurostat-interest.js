...
async function updateInterest({ target = path.join(__dirname, "../lib/fiscal/interest.gen.json"), fetchImpl = fetch, now = () => new Date().toISOString() } = {}) {
  ...
  const responses = Object.fromEntries(await Promise.all(KINDS.map(async kind => {
    ...
    if (!response.ok) throw new Error(`Interest ${kind}: HTTP ${response.status}`);
  ...
  console.log(`Saved government interest: ${snapshot.years[0]}–${snapshot.latestYear}, EU27, matched annual-average population`);
  return snapshot;
}
if (require.main === module) updateInterest().catch(error => { console.error(error); process.exitCode = 1; });
