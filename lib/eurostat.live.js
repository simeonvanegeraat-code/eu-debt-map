...
// Exporteert named + default (werkt met elke importstijl).
//
// Gebruik:
//   import { getLatestGDPForGeoEUR } from "@/lib/eurostat.live";
//   // of
//   import Eurostat from "@/lib/eurostat.live";
...
function parseLatestTimepoint(json) {
  ...
  // Safety voor meerdimensionaal (zou niet moeten voorkomen na filters)
  const multipliers = [];
...
export async function getLatestGDPForGeoEUR(geo) {
  ...
  if (isFresh(hit)) return { valueEUR: hit.valueEUR, period: hit.period, cached: true };

  let out = { valueEUR: null, period: null };
  try { out = await fetchLatestGDP_MioEUR(code); } catch {}
  const entry = { valueEUR: out.valueEUR ?? null, period: out.period ?? null, fetchedAt: now(), ttlMs: DEFAULT_TTL_MS };
...
export async function warmupGDPForCountries(geos = []) {
  ...
  for (const g of Array.isArray(geos) ? geos : []) {
    ...
    try { out[code] = await getLatestGDPForGeoEUR(code); }
    catch { out[code] = { valueEUR: null, period: null, cached: false }; }
  }
  return out;
}
