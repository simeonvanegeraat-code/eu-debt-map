...
import { getTranslationGroupBySlug } from "@/lib/articleTranslations";
...
export function getTranslations(slug) {
  const list = ensureCache();
  const translationGroup = getTranslationGroupBySlug(slug);

  if (!translationGroup) {
    return list.filter((article) => article.slug === slug);
  }

  return list.filter(
    (article) => translationGroup[article.lang] === article.slug
  );
}
