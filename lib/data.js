...
import { EUROSTAT_RATIO_SERIES } from "@/lib/eurostat.ratio.gen";
...
export const countries = EU27.map((code) => ({
  ...
  official_debt_to_gdp_pct: null,
  official_debt_to_gdp_time: null,
  hasOfficialDebtToGDP: false,
...
for (const c of countries) {
  ...
  const ratioRow = EUROSTAT_RATIO_SERIES?.[c.code];
  ...
  if (row && Number.isFinite(row.startValue) && Number.isFinite(row.endValue)) {
    ...
    const prevISO =
      normalizeISODate(row.startDateISO) ||
      quarterKeyToISO(row.previousTime) ||
      c.prev_date;

    const lastISO =
      normalizeISODate(row.endDateISO) ||
  ...
  if (
    ratioRow &&
    Number.isFinite(Number(ratioRow.ratioPct)) &&
    Number(ratioRow.ratioPct) > 0 &&
    ratioRow.latestTime
  ) {
    c.official_debt_to_gdp_pct = Number(ratioRow.ratioPct);
    c.official_debt_to_gdp_time = ratioRow.latestTime;
    c.hasOfficialDebtToGDP = true;
  }
...
function dataQuarterOrdinal(value) {
  const match = /^(\d{4})-?Q([1-4])$/i.exec(String(value || "").trim());
  if (!match) return null;
  return Number(match[1]) * 4 + Number(match[2]) - 1;
}

function summarizeDebtPeriods(list) {
  const periodCounts = {};
  for (const country of list) {
    const period = country?.official_latest_time;
    if (!dataQuarterOrdinal(period)) continue;
    periodCounts[period] = (periodCounts[period] || 0) + 1;
  }

  const ranked = Object.entries(periodCounts).sort((a, b) => {
    if (b[1] !== a[1]) return b[1] - a[1];
    return dataQuarterOrdinal(b[0]) - dataQuarterOrdinal(a[0]);
  });
  const dominantLatestTime = ranked[0]?.[0] || null;
  const latestAvailableTime = Object.keys(periodCounts).sort(
    (a, b) => dataQuarterOrdinal(a) - dataQuarterOrdinal(b)
  ).at(-1) || null;

  return {
    periodCounts,
    dominantLatestTime,
    dominantCoverage: dominantLatestTime ? periodCounts[dominantLatestTime] : 0,
    latestAvailableTime,
  };
}

export const debtDataSummary = summarizeDebtPeriods(countries);

// Een land dat meer dan één kwartaal achterloopt blijft zichtbaar met zijn
// laatste officiële waarde, maar een oude trend wordt niet onbeperkt doorgetrokken.
for (const country of countries) {
  const countryOrdinal = dataQuarterOrdinal(country.official_latest_time);
  const dominantOrdinal = dataQuarterOrdinal(debtDataSummary.dominantLatestTime);
  const periodsBehind =
    Number.isFinite(countryOrdinal) && Number.isFinite(dominantOrdinal)
      ? Math.max(0, dominantOrdinal - countryOrdinal)
      : null;

  country.debtPeriodsBehind = periodsBehind;
  country.isStaleDebtData = Number.isFinite(periodsBehind) && periodsBehind > 0;
  if (Number.isFinite(periodsBehind) && periodsBehind > 1) {
    country._perSecond = 0;
    country.isDebtTickerFrozen = true;
  } else {
    country.isDebtTickerFrozen = false;
  }
}
...
export function interpolateDebt(c, atMs) {
  ...
  const perSec = Number.isFinite(c._perSecond) ? c._perSecond : (perMs * 1000);
  return v1 + perSec * ((atMs - t1) / 1000);
}

export function officialDebtToGDPRatio(c) {
  const ratio = Number(c?.official_debt_to_gdp_pct);
  return Number.isFinite(ratio) && ratio > 0 ? ratio : null;
}

export function estimatedLiveDebtToGDPRatio(c, atMs = Date.now()) {
  const officialRatio = officialDebtToGDPRatio(c);
  const officialDebt = Number(c?.last_value_eur);
  const debtPeriod = String(c?.official_latest_time || "");
  const ratioPeriod = String(c?.official_debt_to_gdp_time || "");

  if (
    !Number.isFinite(officialRatio) ||
    !Number.isFinite(officialDebt) ||
    officialDebt <= 0 ||
    !debtPeriod ||
    debtPeriod !== ratioPeriod
  ) {
    return null;
  }

  const liveDebt = interpolateDebt(c, atMs);
  return Number.isFinite(liveDebt) && liveDebt > 0
    ? officialRatio * (liveDebt / officialDebt)
    : null;
}
