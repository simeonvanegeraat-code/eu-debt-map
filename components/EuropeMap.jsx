...
// Map landnaam -> ISO2 code dat in onze data voorkomt
const NAME_TO_ISO2 = {
  ...
  return (
    <ComposableMap projection="geoAzimuthalEqualArea" projectionConfig={{ rotate: [-10,-52,0], scale: 900 }}>
      <Geographies geography={GEO_URL}>
        {({ geographies }) =>
          geographies
            // ❌ geen bbox meer; ✅ alleen landen die we herkennen op naam
            .filter((geo) => {
            ...
            .map((geo) => {
              ...
              const hasData = !!data.find(c => c.code.toUpperCase() === code2);
              return (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
                  style={{
                    default:{ fill: hasData ? fillFor(code2) : "rgba(255,255,255,0.06)", outline:"none" },
                    hover:{ fill:"#60a5fa", outline:"none" },
                    pressed:{ outline:"none" }
                  }}
                  onClick={()=>{
                    if (hasData) window.location.href = `/country/${code2.toLowerCase()}`;
                  }}
                />
              );
            })
