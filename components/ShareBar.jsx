...
function Icon({ name, size = 18 }) {
  ...
  if (name === "reddit") {
    return (
      <svg {...common}><path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm-3.667-9.404a1.867 1.867 0 1 0-.001-3.734 1.867 1.867 0 0 0 .001 3.734zm7.334 0a1.867 1.867 0 1 0-.001-3.734 1.867 1.867 0 0 0 .001 3.734zm-9.041 3.327a.546.546 0 0 0-.022.77c.717.784 2.458 1.874 5.396 1.874 2.939 0 4.68-1.09 5.396-1.874a.546.546 0 1 0-.805-.736c-.443.483-1.748 1.272-4.591 1.272-2.845 0-4.148-.79-4.591-1.272a.546.546 0 0 0-.783-.034z"/></svg>
    );
  }
  if (name === "linkedin") {
    return (
...
export default function ShareBar({ title, summary }) {
  ...
  const whatsAppUrl = useMemo(() => {
    ...
    if (url) u.searchParams.set("text", `${title} ${url}`);
    return u.toString();
  }, [url, title]);
